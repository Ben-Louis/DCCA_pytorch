...
class CCA_torch:
  def __init__(self, output_dim=1, use_all_singular_values=True, cca_space_dim=10, use_cuda=True):
  ...
  def call(self, x):
    ...
    if False:
      SigmaHat11 += r1 * torch.diag(torch.ones(o1)).type(type(x)).cuda()
      SigmaHat22 += r2 * torch.diag(torch.ones(o2)).type(type(x)).cuda()
    else:
      SigmaHat11 += r1 * torch.diag(torch.ones(o1)).type(type(x))
      SigmaHat22 += r2 * torch.diag(torch.ones(o2)).type(type(x))     
    ...
    D1 = D1[D1_indices]
    D2 = D2[D2_indices]    
    tmp = torch.ByteTensor([1]*V1.size(1))  
    if False:
      tmp = tmp.cuda()
    
    D1_indices = D1_indices.view(-1,1).matmul(tmp.view(1,-1)).transpose(0,1)
    D2_indices = D2_indices.view(-1,1).matmul(tmp.view(1,-1)).transpose(0,1)
